...
mod tests {
    ...
    fn test_f0_method_conversion() {
        assert_eq!(F0Method::from_str("dio"), Some(F0Method::Dio));
        ...
        assert_eq!(F0Method::Dio.to_str(), "dio");
        assert_eq!(F0Method::Harvest.to_str(), "harvest");
        assert_eq!(F0Method::Pm.to_str(), "pm");
    }

    #[test]
    fn test_dio_predictor() -> RvcResult<()> {
        let predictor = DioF0Predictor::with_default();
        ...
        let f0 = predictor.compute_f0(&audio, None)?;
        assert!(!f0.is_empty());
    ...
    #[test]
    fn test_harvest_predictor() -> RvcResult<()> {
        let predictor = HarvestF0Predictor::with_default();
        let sample_rate = 16000;
        ...
        // 需要更长的音频来进行有效的F0估计
        let mut extended_audio = Vec::new();
        for _ in 0..1000 {
            extended_audio.extend_from_slice(&audio);
        }

        let f0 = predictor.compute_f0(&extended_audio, None)?;
        assert!(!f0.is_empty());
    ...
    #[test]
    fn test_pm_predictor() -> RvcResult<()> {
        let predictor = PmF0Predictor::with_default();
        let sample_rate = 16000;
    ...
    #[test]
    fn test_f0_predictor_factory() -> RvcResult<()> {
        let dio_predictor = F0PredictorFactory::create_default(F0Method::Dio)?;
        assert_eq!(dio_predictor.method(), F0Method::Dio);

        let harvest_predictor = F0PredictorFactory::create_default(F0Method::Harvest)?;
        assert_eq!(harvest_predictor.method(), F0Method::Harvest);

        let pm_predictor = F0PredictorFactory::create_default(F0Method::Pm)?;
        assert_eq!(pm_predictor.method(), F0Method::Pm);

        Ok(())
    }

    #[test]
    fn test_f0_uv_computation() -> RvcResult<()> {
        let predictor = DioF0Predictor::with_default();
        ...
        let (f0, uv) = predictor.compute_f0_uv(&audio, None)?;
        assert_eq!(f0.len(), uv.len());
        assert!(!f0.is_empty());
        assert!(!uv.is_empty());
